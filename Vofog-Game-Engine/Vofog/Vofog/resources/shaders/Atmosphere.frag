...
uniform VFG_SpotLight u_VFG_SpotLight[VFG_SPOT_LIGHT_COUNT];
...
vec3 spotLightCalculation(VFG_SpotLight light, vec4 diffuseMap, vec4 specularMap, vec3 normal, vec3 viewDir){
	...
	diffuse*= attenuation * softness; specular*= 1*attenuation * softness;ambient*= attenuation* attenuation;
...
void main(){
	...
	for(int i =0; i < VFG_POINT_LIGHT_COUNT; i++)
		...
		vec3 thing = vec3(0);
	for(int i =0; i < VFG_SPOT_LIGHT_COUNT; i++)
		thing += spotLightCalculation(u_VFG_SpotLight[i],  diffuseMap, specularMap, norm, viewDir); 
	vec3 gamma = vec3(1.0);
	if(u_useGammaModifier) gamma = vec3(1.0f/2.2f);
